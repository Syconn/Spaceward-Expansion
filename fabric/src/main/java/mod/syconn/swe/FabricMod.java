...
public class FabricMod implements ModInitializer {
    ...
    public void onInitialize() {
        FluidStorage.GENERAL_COMBINED_PROVIDER.register(context -> {
            if (context.getItemVariant().getItem() instanceof Canister) return new ComponentFluidWrapper(context, 8000);
            return null;
        });
        FluidStorage.SIDED.registerForBlockEntities((blockEntity, context) -> {
            if (blockEntity instanceof AbstractTankBE be) return new BlockFluidWrapper(be.getFluidTank());
            return null;
        }, BlockEntityRegister.DISPERSER.get());
        FluidStorage.SIDED.registerForBlockEntities((blockEntity, context) -> {
            if (blockEntity instanceof AbstractTankBE be) return new InteractableFluidWrapper(be.getFluidTank());
            return null;
        }, BlockEntityRegister.TANK.get(), BlockEntityRegister.COLLECTOR.get());

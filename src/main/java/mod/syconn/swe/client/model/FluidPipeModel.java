...
public class FluidPipeModel extends Model {
	public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(Main.loc("fluidpipemodel"), "main");
	...
	public void renderCase(BlockState state, PoseStack poseStack, VertexConsumer vertexConsumer, int packedLight, int packedOverlay) {
		...
		if (mod.isUp()) bot.render(poseStack, vertexConsumer, packedLight, packedOverlay, -1);
		if (mod.isDown()) top.render(poseStack, vertexConsumer, packedLight, packedOverlay, -1); // TODO Old was (1, 1, 1, 1)
	}

	public void renderFluid(BlockState state, PoseStack poseStack, VertexConsumer vertexConsumer, int packedLight, int packedOverlay, int color) {
		PipeModule mod = new PipeModule(state);
		if (mod.isUp() && mod.hasFluid()) bot_fluid.render(poseStack, vertexConsumer, packedLight, packedOverlay, color);
		if (mod.isDown() && mod.hasFluid()) top_fluid.render(poseStack, vertexConsumer, packedLight, packedOverlay, color);
	}

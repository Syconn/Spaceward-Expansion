...
public class BlockRegister {
    ...
    public static final Supplier<Block> DISPERSED_OXYGEN = registerBlock("dispersed_oxygen", () -> new DispersedAirBlock(Blocks.AIR.properties().noCollission().noLootTable().air().isViewBlocking((state, level, pos) -> false)));
    public static final Supplier<Block> O2 = registerBlock("oxygen", () -> new FluidBlock(FluidRegister.O2.get(), Blocks.LAVA.properties().lightLevel(blockstate -> 0)));
    ...
    public static final Supplier<MapCodec<FluidPipe>> FlUID_PIPE_CODEC = registerCodec("fluid_pipe", () -> simpleCodec(FluidPipe::new));
    ...
    private static <T extends Block> Supplier<T> registerBlock(String id, Supplier<T> block) {
        return Services.REGISTRAR.registerBlock(id, block);
    }
